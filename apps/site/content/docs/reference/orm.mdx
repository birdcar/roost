---
title: "@roost/orm"
description: Laravel-inspired ORM built on Drizzle for D1 databases. Model classes, query builder, relationships, lifecycle hooks, factories.
---
...
```ts
import { Model, HasManyRelation, BelongsToRelation } from '@roost/orm';
...
| Parameter | Description |
|---|---|
| `resolver` | Any object implementing `resolve(request: Request): { slug: string } \| null`. `OrgResolver` from `@roost/auth` satisfies this interface. |
