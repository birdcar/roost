...
Routes inference requests through [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) instead of the direct Workers AI binding. Adds observability, request caching, and automatic fallback.
...
Check the status of a queued async inference task via the Cloudflare REST API. Located in `@roost/cloudflare`.
