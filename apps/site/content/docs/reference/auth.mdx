...
`TenantScopeMiddleware` is exported from `@roostjs/orm`, not `@roostjs/auth`, but it is designed to be wired together with `OrgResolver` from `@roostjs/auth`. See the full API reference in the [@roostjs/orm reference — TenantScopeMiddleware](/docs/reference/orm#tenantscopemiddleware).
