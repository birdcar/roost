...
description: HTTP test client, response assertions, unified fakes, and test setup utilities for Roost applications on bun:test.
---

> **Testing AI agents?** `@roostjs/ai/testing` ships its own fakes and
> harnesses — `Agent.fake()`, `preventStrayPrompts()`, auto-fake structured
> output, and `TestStatefulAgentHarness` for DO-backed agents. See
> [AI reference → Testing](/docs/reference/ai#testing) for agent-side APIs.
