...
`EventDispatcher` when `@roost/broadcast` is installed.
...
```ts title="client entrypoint: @roost/broadcast/client"
...
`@roost/broadcast/client` — this module does not import any server-side code.
