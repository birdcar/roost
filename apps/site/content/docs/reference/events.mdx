---
title: "@roost/events"
description: In-process event dispatching with listener registration, subscriber classes, optional queue integration, and broadcast interop.
---

## Installation

```bash title="terminal"
bun add @roost/events
```
...
`BroadcastManager` if `@roost/broadcast` is installed.

#### `static fake(): void`

Enable fake mode for this event class. Dispatches are recorded but listeners
are not called.
...
Return the current singleton dispatcher. Creates a default instance if none has
been set.
...
Register a listener class for an event class. Called during bootstrap by the
service provider.

#### `async dispatch(event: Event): Promise<void>`

Instantiate and call all listeners registered for `event`'s class. If a listener
implements `ShouldQueue`, the listener is dispatched as a Job via `@roost/queue`
...
from `@roost/queue` to have the listener dispatched as a background job instead
of called inline. Requires `@roost/queue` to be installed.
...
Return a map of event class to method name string. The method must exist on the
subscriber instance.
...
Override to return a map of event classes to arrays of listener classes.
Defaults to an empty map.

#### `protected subscribers(): SubscriberClass[]`

Override to return an array of subscriber classes to register.
