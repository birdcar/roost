...
`@roost/ai` package pre-installed and the Cloudflare Workers AI binding already
...
Run the model generator to create a `ChatMessage` model and its migration file:

```terminal title="terminal"
roost make:model ChatMessage
```

This creates two files: `src/models/ChatMessage.ts` and a timestamped migration
in `database/migrations/`. Open the migration and add the columns your chat app
needs:

```ts title="database/migrations/XXXX_create_chat_messages_table.ts"
...
**You should see** output confirming the migration ran successfully, e.g.
`Migrated: XXXX_create_chat_messages_table`. The
...
```ts title="src/agents/ChatAssistant.ts"
...
```ts title="src/agents/tools/SummarizeTool.ts"
...
- Persists every message to a D1 database using `@roost/orm`
...
- [@roost/ai reference](/docs/reference/ai) — full API documentation for `Agent`, `Tool`, decorators, and the provider interface
