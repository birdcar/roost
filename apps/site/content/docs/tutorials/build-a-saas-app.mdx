...
<Callout type="note">
  ...
  <ul>
    ...
    <li>Gating premium features with <code>SubscribedMiddleware</code> from <code>@roostjs/billing</code></li>
    <li>Creating Stripe Checkout sessions and verifying webhooks</li>
    <li>Dispatching background jobs with <code>@roostjs/queue</code></li>
  </ul>
  ...
  <p><strong>Packages used:</strong> <code>@roostjs/auth</code>, <code>@roostjs/billing</code>, <code>@roostjs/orm</code>, <code>@roostjs/start</code>, <code>@roostjs/queue</code></p>
</Callout>
...
This creates `src/models/workspace.ts` with a stub. Open it and replace the
placeholder columns with the schema your SaaS app needs. Each workspace belongs
to a WorkOS organization:

```ts title="src/models/workspace.ts"
...
- [@roostjs/auth reference](/docs/reference/auth) — full API for `SessionManager`, `OrgResolver`, and role-based middleware
- [@roostjs/billing guide](/docs/guides/billing) — subscription swaps, usage-based billing, the customer portal, and webhook event catalogue
