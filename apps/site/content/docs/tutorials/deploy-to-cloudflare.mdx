...
You should see bindings for `DB` (D1 database), `KV` (key-value store), and `AI` (Workers AI). The `database_id` and KV `id` fields are filled in automatically when you run `roost new`. If
they're still placeholder values, run `roost provision` to create the resources and populate the file.
