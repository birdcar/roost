---
title: "@roostjs/schema"
description: Why Roost has a schema builder, what it produces, and why the same schema definitions power both AI tools and MCP tools.
---
...
keys, and no IDE assistance for schema-specific properties like `minimum` or
`enum`.

The `schema` builder provides a TypeScript-native API for constructing these
JSON Schema objects. `schema.string().minLength(2).description('User name')`
...
The builder is transparent: it produces plain JSON Schema objects. Calling `.build()`
on any builder returns the `JsonSchemaOutput` it describes. There is no special
...
Both `@roostjs/ai` and `@roostjs/mcp` use the same schema builder for
...
`schema.string()` does not produce a TypeScript type `string` that
...
- [@roostjs/ai concepts — how tool schemas are used by agents](/docs/concepts/ai)
- [@roostjs/mcp concepts — how MCP tools use schema definitions](/docs/concepts/mcp)
- [@roostjs/schema reference — all builder methods and types](/docs/packages/schema)
