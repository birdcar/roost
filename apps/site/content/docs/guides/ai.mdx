...
description: "Task-oriented instructions for agents, tools, streaming, queueing, sessions, workflows, sub-agents, MCP, HITL, RAG, and testing."
---
...
Each instance holds its own in-memory conversation history. For durable
cross-request state, upgrade to [`StatefulAgent`](#how-to-persist-conversation-history).
See [reference → Prompting](/docs/reference/ai#prompting).
...
```ts title="src/agents/resilient.ts"
...
See [reference → Failover](/docs/reference/ai#failover).
...
```ts title="src/agents/research.ts"
...
See [reference → Tools](/docs/reference/ai#tools).
...
See [reference → Streaming](/docs/reference/ai#streaming).
...
```ts title="src/agents/summarize.ts"
...
```ts title="src/routes/api/summarize.ts"
...
See [reference → Queueing](/docs/reference/ai#queueing).
...
```ts title="src/agents/support-do.ts"
...
See [reference → Sessions](/docs/reference/ai#conversation-context-sessions).
...
See [reference → Schedule](/docs/reference/ai#schedule).
...
```ts title="src/agents/report-agent.ts"
import { StatefulAgent } from '@roostjs/ai/stateful';
...
See [reference → Workflows](/docs/reference/ai#workflows) and
[`@roostjs/workflow` reference](/docs/reference/workflow).
...
```ts title="src/agents/orchestrator.ts"
import { StatefulAgent } from '@roostjs/ai/stateful';
...
[reference → Sub-agents](/docs/reference/ai#sub-agents).
...
See [reference → MCP](/docs/reference/ai#mcp).
...
```ts title="src/agents/treasurer.ts"
import { StatefulAgent } from '@roostjs/ai/stateful';
...
See [reference → HITL](/docs/reference/ai#hitl).
...
```ts title="src/agents/docs-agent.ts"
import { Agent } from '@roostjs/ai';
...
See [reference → Vector Stores](/docs/reference/ai#vector-stores).
...
```ts title="tests/support.test.ts"
...
See [reference → Testing](/docs/reference/ai#testing).
