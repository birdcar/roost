---
title: "@roost/orm Guides"
description: Task-oriented instructions for models, querying, relationships, factories, and testing.
---
...
```ts title="src/middleware/TenantMiddleware.ts"
import { TenantScopeMiddleware } from '@roost/orm';
import { OrgResolver } from '@roost/auth';
import { TenantContext } from '@roost/orm';
...
app.useMiddleware(makeTenantMiddleware(app.container.resolve(TenantContext)));
```

### 4. Use the escape hatch for cross-tenant queries
...
## How to set up DB-per-tenant routing
...
import { D1SessionHandle } from '@roost/orm';
...
const updated = await Post.find(post.attributes.id); // guaranteed to see the new row
```

If `withSession()` is unavailable (e.g. local dev with Miniflare), the handle falls back to the plain binding and logs a warning.
