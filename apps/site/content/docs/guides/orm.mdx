...
description: Task-oriented instructions for models, migrations, querying, relationships, and testing.
---
...
See [the migrations guide](/docs/guides/migrations) for the full column reference and rollback patterns.
...
Declare relationships as static method calls. They are lazy-loaded — call the method on an instance to fetch related records.
...
const posts = await (await User.findOrFail(1)).posts(); // Post[]
```
...
Create seeder files in `database/seeders/` and run them with `roost db:seed`.

```ts database/seeders/UserSeeder.ts
import { Seeder } from '@roost/orm';
...
Define a factory for a model and use it in tests to create records without specifying every attribute.
