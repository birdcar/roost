...
## How to add structured logging with request tracing
...
Resolve the logger inside a handler or downstream middleware:

```ts title="src/handlers/users.ts"
import { Logger } from '@roost/core';
...
```ts title="src/handlers/users.test.ts"
import { Logger } from '@roost/core';
...
```ts title="src/routes/webhooks/stripe.ts"
...
// Wire into a route-level pipeline or app.useMiddleware() for a prefix group
```

Available presets: `WebhookPresets.stripe()`, `WebhookPresets.github()`,
`WebhookPresets.svix()`.
...
```ts title="src/routes/webhooks/custom.ts"
import { verifyWebhook, WebhookVerificationError } from '@roost/core/webhooks/verify';
