...
The `register()` method runs before all `boot()` calls, so you can safely resolve services registered by other providers inside `boot()`. See [@roostjs/core reference](/docs/packages/core) for full ServiceProvider API.
