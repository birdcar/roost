---
title: "@roost/workflow Guides"
description: Task-oriented instructions for defining workflows, running durable steps, implementing sagas with Compensable, and testing without Cloudflare.
---
...
```ts title="src/workflows/ProvisionTenant.ts"
import { Workflow, NonRetryableError } from '@roost/workflow';
...
```ts title="src/providers/AppServiceProvider.ts"
import { WorkflowServiceProvider } from '@roost/workflow';
...
```ts title="src/handlers/organizations.ts"
import type { AppContainer } from '../container';
import type { WorkflowClient } from '@roost/workflow';
...
```ts title="src/workflows/OnboardUser.ts"
import { Workflow, Compensable } from '@roost/workflow';
import type { WorkflowEvent, WorkflowStep } from 'cloudflare:workers';
...
Compensations run in reverse registration order. Errors in individual
compensations are swallowed so the remaining compensations still execute.
...
// Terminate
await client.terminate(workflowId);
```

## How to test workflows without Cloudflare
...
```ts title="tests/workflows/ProvisionTenant.test.ts"
