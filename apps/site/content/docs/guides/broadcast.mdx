---
title: "@roost/broadcast Guides"
description: Task-oriented instructions for setting up broadcasting, creating broadcastable events, connecting a client, authorizing private channels, and testing.
---
...
import { BroadcastServiceProvider } from '@roost/broadcast';
...
`@roost/events`, extend `Event` as well to get in-process listener support
alongside broadcasting.

```ts title="src/events/OrderShipped.ts"
import { Event } from '@roost/events';
import { PrivateChannel } from '@roost/broadcast';
import type { BroadcastableEvent } from '@roost/broadcast';
...
Dispatch the event normally. If `@roost/events`' `EventDispatcher` is
...
```ts
import { BroadcastManager, Channel } from '@roost/broadcast';
...
Use `createBroadcastClient` from `@roost/broadcast/client` in browser or client-side code.

```ts title="src/client/notifications.ts"
import { createBroadcastClient } from '@roost/broadcast/client';
...
The client reconnects automatically with exponential backoff when the connection
drops. Pass a factory function as the URL to generate a fresh token on each reconnect.
...
```ts title="src/durable-objects/AppChannelDO.ts"
import { ChannelDO } from '@roost/broadcast';
...
```ts title="tests/events/OrderShipped.test.ts"
import { describe, it, afterEach } from 'bun:test';
import { BroadcastManager } from '@roost/broadcast';
