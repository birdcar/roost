---
title: "@roost/cloudflare Guides"
description: "Task-oriented instructions for Cloudflare bindings: D1, R2, KV, Queues, Workers AI, and Vectorize."
---
...
For application-level querying, prefer [@roost/orm](/docs/packages/orm) models over raw D1. Use raw D1 for migrations or complex SQL not covered by the query builder.
...
```ts
import { R2Storage } from '@roost/cloudflare';
...
Use `QueueSender` for direct queue access, or use [@roost/queue](/docs/packages/queue) jobs for structured background processing.
...
For full agent capabilities with tools and conversation memory, use [@roost/ai](/docs/packages/ai). `AIClient` is the low-level binding wrapper.
...
```ts
import { VectorStore, AIClient } from '@roost/cloudflare';
...
// Delete by IDs
await vectorize.deleteByIds(['doc_1', 'doc_2']);
```

The Vectorize index dimension must match your embedding model's output size. `bge-base-en-v1.5` outputs 768 dimensions.
...
```ts title="src/handlers/checkout.ts"
import { ServiceClient } from '@roost/cloudflare';
...
```ts title="src/handlers/tenant.ts"
import { DispatchNamespaceClient } from '@roost/cloudflare';
...
const response = await tenantClient.get('/status');
```

For full control, use `dispatch()` to get the raw `Fetcher`:

```ts
const fetcher = dispatch.dispatch(tenantScriptName, { trust: 'trusted' });
const response = await fetcher.fetch('https://worker/path');
```

## How to communicate with a Cloudflare Container
...
```ts title="src/handlers/render.ts"
import { ContainerClient } from '@roost/cloudflare';
...
```ts title="src/middleware/html-enrichment.ts"
import { HtmlTransformer } from '@roost/cloudflare';
...
For A/B testing, provide a variant map and an assignment function that reads a cookie or
header to select the variant:
...
return transformer.transform(response, request);
```

## How to use VersionedKVStore for content-addressed caching
...
```ts title="src/services/config-cache.ts"
import { VersionedKVStore, KVStore } from '@roost/cloudflare';
...
import { KVRateLimiter, KVStore } from '@roost/cloudflare';
...
```ts title="src/app.ts"
import { DORateLimiter, DurableObjectClient } from '@roost/cloudflare';
...
export { RateLimiterDO } from '@roost/cloudflare';
```
...
```ts title="src/handlers/api.test.ts"
import { fakeRateLimiter, restoreRateLimiter } from '@roost/cloudflare';
