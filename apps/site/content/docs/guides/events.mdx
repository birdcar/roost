---
title: "@roost/events Guides"
description: Task-oriented instructions for defining events, registering listeners, using subscribers, deferring listeners to the queue, and testing dispatches.
---
...
```ts title="src/events/UserRegistered.ts"
...
```ts title="src/listeners/SendWelcomeEmailListener.ts"
...
```ts title="src/providers/AppEventServiceProvider.ts"
import { EventServiceProvider } from '@roost/events';
...
All registered listeners for `UserRegistered` execute in parallel before
`dispatch()` resolves.
...
```ts title="src/subscribers/NotificationSubscriber.ts"
import { Subscriber } from '@roost/events';
import type { EventClass, Event } from '@roost/events';
import { UserRegistered } from '../events/UserRegistered';
...
from `@roost/queue`. The event dispatcher will dispatch the listener as a job
rather than calling `handle()` inline.

```ts title="src/listeners/GenerateInvoiceListener.ts"
...
Requires `@roost/queue` to be installed and configured.
...
```ts title="tests/events/UserRegistered.test.ts"
