...
- [ ] `@roost/ai` supports AI Gateway proxy, async inference (`queueRequest`), prefix caching (`x-session-affinity`), and provider fallback chains
- [ ] A `RAGPipeline` class exists that handles chunking, embedding, Vectorize insert, and retrieval with configurable strategies
- [ ] `@roost/mcp` can expose AI Search instances as MCP resources
- [ ] Workflows have a Roost abstraction (`@roost/workflow` or integrated into `@roost/queue`) with a `roost make:workflow` generator
...
- New packages: `@roost/workflow`, `@roost/observability`, `@roost/feature-flags`, `@roost/events`, `@roost/broadcast`
- Extensions to existing packages: `@roost/ai`, `@roost/cloudflare`, `@roost/orm`, `@roost/auth`, `@roost/mcp`, `@roost/core`
...
| MCP + AI Search | `AiSearchResource` for `@roost/mcp` that exposes AI Search instances as queryable resources |
...
| Gap | Deliverable |
|---|---|
| Workflows | `@roost/workflow` package with `Workflow` base class, step helpers, compensation patterns |
...
| Gap | Deliverable |
|---|---|
| Events | `@roost/events` — `Event.dispatch()`, listeners (sync + queued), `EventServiceProvider`, subscriber pattern |
| Broadcasting | `@roost/broadcast` — `BroadcastableEvent`, `PrivateChannel`/`PresenceChannel`, `ChannelDO` with WebSocket hibernation |
