...
| `packages/start/package.json` | @roost/start package manifest |
...
| `packages/start/src/provider.ts` | StartServiceProvider for @roost/core |
...
| `packages/core/src/index.ts` | Export new types needed by @roost/start |
...
│   ├── core/
│   ├── cloudflare/
│   └── start/                # @roost/start — new this phase
└── apps/
...
import { withRoost } from '@roost/start';
...
**Overview**: A service provider that Phase 3 and later phases can register, giving them access to the Roost context bridge without direct dependency on `@roost/start` internals.
